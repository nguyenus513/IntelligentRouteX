...
def run_one_subproblem(instance: Dict[str, Any], incumbent: Dict[str, Any], request_limit: int, sub_time_ms: int) -> Dict[str, Any]:
    ...
    improves = bool(checked.get("feasible")) and exact_request_coverage(instance, recombined) and objective.improves(instance, incumbent, recombined)
    ...
    return {"selectedRequestCount": len(selected), "subproblemRequestCount": len(subproblem.get("requests", [])), "subDiagnostics": result.get("diagnostics", {}), "recombinedSolution": recombined, "recombinedFeasible": bool(checked.get("feasible")), "recombinedViolations": checked.get("violations", []), "exactCoverage": exact_request_coverage(instance, recombined), "objectiveImproves": improves, "objectiveDelta": objective_delta}
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for index in range(max(1, args.subproblem_count)):
        if int((time.perf_counter() - started) * 1000) > args.hard_wall_clock_ms:
            break
        item = run_one_subproblem(instance, incumbent, args.request_limit, parse_time_limit(args.subproblem_time_limit))
    ...
    rejected_hard = sum(0 if item.get("recombinedFeasible") else len(item.get("recombinedViolations", [])) for item in trace)
    ...
    summary = {"schemaVersion": "phase93-lilim-decomposition-probe/v1", "gate": gate, "instance": instance_name, "decompositionTrace": [{key: value for key, value in item.items() if key not in {"subDiagnostics", "recombinedSolution"}} for item in trace], "subproblemCount": len(trace), "recombinedCandidates": recombined, "acceptedRecombinedCandidates": accepted, "unknownCount": unknown, "hardViolations": hard, "rejectedRecombinedHardViolations": rejected_hard, "antiHardcodeGate": anti.get("gate"), **telemetry}
