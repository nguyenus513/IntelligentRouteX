...
def download_text(url: str, path: Path) -> dict[str, Any]:
    ...
    return {"url": url, "path": str(path), "bytes": len(payload)}
...
def download_icaps(root: Path, instances: Iterable[str]) -> list[dict[str, Any]]:
    entries: list[dict[str, Any]] = []
    ...
    for local_name in instances:
        folder, orders_file, vehicle_file = ICAPS_SMOKE_INSTANCES[local_name]
...
def write_homberger_manifest(root: Path) -> dict[str, Any]:
    ...
    return {"benchmark": "homberger-vrptw", "path": str(manifest_path), **HOMBERGER_DOWNLOAD_NOTES}
...
def main() -> int:
    ...
    manifest: dict[str, Any] = {"schemaVersion": "certification-benchmark-download-manifest/v1", "entries": []}
    if "mdrplib" in groups:
        manifest["entries"].extend(download_mdrplib(root, MDRPLIB_SMOKE_INSTANCES.keys()))
    if "icaps" in groups:
        manifest["entries"].extend(download_icaps(root, ICAPS_SMOKE_INSTANCES.keys()))
